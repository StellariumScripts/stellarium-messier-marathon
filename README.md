...
<br>
<br>
Here is a small example of what the Stellarium CSV output file looks like in a spreadsheet program:

![Brief example of what a CSV file automatically generated by Stellarium looks like](images/csv.png)
...
* [Fall Messier Marathon](#fall-messier-marathon)
* [2027 Messier Marathon Information](#2027-messier-marathon-information)
* Instructions
...
* FAQ
    ...
    * [I edited the script to have it generate the CSV and TXT files.  Where are they located?](#i-edited-the-script-to-have-it-generate-the-csv-and-txt-files--where-are-they-located)
    * [I edited the script to have it generate the CSV and TXT files.  How do I use them?](#i-edited-the-script-to-have-it-generate-the-csv-and-txt-files--how-do-i-use-them)
...
There is another opportunity to _almost_ complete a full Messier Marathon every fall season. It is important to know that it will not be possible for anyone at any location to see all 110 Messier objects in a single night in the fall, but it may be possible to see _nearly_ 110 objects.
...
# 2027 Messier Marathon Information
...
An attempt at a full marathon can always be made later in March 2027 or very early in April 2027. But, the moonlight that will be shining during these nights may greatly reduce your chances of seeing some of the fainter Messier objects either after sunset or before sunrise. If an ill-advised attempt is to be made at an extremely challenging full marathon around the new moon phase on April 6, the latitude of the location should actually be much higher north around this date - approximately 36°N to 43°N.
...
* Download the `messier_marathon_simulation.ssc` script file directly anywhere on your computer:
...
* The spreadsheet will contain the valuable time schedule of when to find each of the objects and has information on each of the objects.


### Additional Option

A further option is to have the script automatically save the final output to two different text files:
...
Yes. While the script is running, the Stellarium GUI disappears and does not reappear until the moment the script finishes.  If the GUI is needed while the script is running, simply use this 2-key combination on your keyboard:<br>
`CTRL` + `t`
<br>
<br>
...
## I edited the script to have it generate the CSV and TXT files.  Where are they located?
...
## I edited the script to have it generate the CSV and TXT files.  How do I use them?

If you set the `saveToOutputFiles` variable in the script to `yes`, then the script will generate two text files (CSV and TXT) at the completion of the script.
...
* In the spreadsheet program, any remaining columns may be shifted either to the left or right of each other.
...
Yes! Take a look around in the `messier_marathon_simulation.ssc` script file for a few undocumented variables you may wish to use.
<br>
<br>
